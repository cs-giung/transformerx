...
@dataclass
class PowerSymmetricQuantizedArray(ImplicitArray):
    """
    PowerQuant: Automorphism Search for Non-Uniform Quantization
    https://arxiv.org/abs/2301.09858
    """
    # pylint: disable=too-many-instance-attributes
    q_value: ArrayLike
    scale_factor: ArrayLike
    ...
    def quantize(
        ...
        arr = arr.transpose(*chain(
            ...
            [contraction_axis]))
        arr = arr.reshape(-1, group_size, arr.shape[-1])
        arr = jnp.sign(arr) * jnp.power(jnp.abs(arr), exponent)
    ...
    def dequantize(
        ...
        arr = q_value * scale_factor[:, None, None]
        arr = jnp.sign(arr) * jnp.power(jnp.abs(arr), 1.0 / exponent)
        arr = arr.reshape(
            shape[:contraction_axis]
            + shape[contraction_axis+1:]
            + (shape[contraction_axis],))
